 ...
 * our headroom for the next scanning cycle by this multiple.
 */
#define	L2ARC_HEADROOM_BOOST	2
...
unsigned long l2arc_compress_minsz = 2048;          /* minimum input size */
...
struct l2arc_buf_hdr {
	...
	enum zio_compress	b_compress;	/* applied compression */
	ssize_t			b_asize;	/* alloc'd size (compressed) */
	void			*b_cdata;	/* temporary compressed data */
};
...
static void l2arc_compress_buf(l2arc_buf_hdr_t *l2hdr);
		...
		if (vd != NULL && l2arc_ndev != 0 && !(l2arc_norw && devw)) {
			...
			if (hdr->b_l2hdr != NULL &&
				...
				if (hdr->b_l2hdr->b_compress ==
				    ZIO_COMPRESS_EMPTY)
					rzio = zio_read_null(pio, vd, addr,
					    hdr->b_l2hdr->b_asize,
...
arc_release(arc_buf_t *buf, void *tag)
{
	...
	if (l2hdr) {
		...
		ARCSTAT_INCR(arcstat_l2_asize, -buf_asize);
		mutex_exit(&l2arc_buflist_mtx);
	}
}
 ...
 *				scanning, we multiply headroom by this factor
 *				for the next scan cycle, since more compressed
 *				buffers are likely to be present
...
l2arc_write_done(zio_t *zio)
{
	...
	for (ab = list_prev(buflist, head); ab; ab = ab_prev) {
		...
		/*
		 * If compression was enabled on this buffer, release the
		 * temporary compressed buffer as soon as possible.
		 */
    ...
    boolean_t *headroom_boost)
{
	...
	list_t *list;
	uint64_t write_asize, write_psize, write_sz, headroom, buf_minsz;
	void *buf_data;
	...
	*headroom_boost = B_FALSE;
	...
	/* We want at least 12.5% compression, no sense in trying otherwise */
	buf_minsz = vdev_psize_to_asize(dev->l2ad_vdev, l2arc_compress_minsz);
	buf_minsz += buf_minsz >> 3;
	...
	for (try = 0; try <= 3; try++) {
		...
		for (; ab; ab = ab_prev) {
			...
			ab->b_flags |= ARC_L2_WRITING;
			/*
			 * Temporarily stash the data buffer in b_cdata. The
			 * subsequent write step will pick it up from there.
			 * This is because can't access ab->b_buf without
			 * holding the hash_lock, which we in turn can't
			 * access without holding the ARC list locks (which
			 * we want to avoid during compression/writing).
			 */
			l2hdr->b_compress = ZIO_COMPRESS_OFF;
			l2hdr->b_cdata = ab->b_buf->b_data;
	...
	for (ab = list_prev(dev->l2ad_buflist, head); ab;
		...
		if ((ab->b_flags & ARC_L2COMPRESS) &&
		    l2hdr->b_asize >= buf_minsz) {
			l2arc_compress_buf(l2hdr);
			/* Enable headroom boost on next scan. */
 ...
 * The data to be compressed must be prefilled in l2hdr->b_cdata and its
 * size in l2hdr->b_asize. This routine tries to compress the data and
 * depending on the compression result there are three possible outcomes:
 * *) The buffer was incompressible. The original l2hdr contents were left
 *    untouched and are ready for writing to an L2 device.
 * *) The buffer was all-zeros, so there is no need to write it to an L2
 *    device. To indicate this situation b_cdata is NULL'ed, b_asize is set
 *    to zero and b_compress is set to ZIO_COMPRESS_EMPTY.
 * *) Compression succeeded and b_cdata was replaced with a temporary data
 *    buffer which holds the compressed data to be written, and b_asize
...
l2arc_compress_buf(l2arc_buf_hdr_t *l2hdr)
{
	...
	ASSERT(l2hdr->b_cdata != NULL);
	ASSERT(l2hdr->b_asize >= l2arc_compress_minsz);

	len = l2hdr->b_asize;
	cdata = zio_data_buf_alloc(len);
	csize = zio_compress_data(ZIO_COMPRESS_LZ4, l2hdr->b_cdata, cdata,
	    l2hdr->b_asize);
	...
	if (csize == 0) {
		...
		/*
		 * Compression failed, release the compressed buffer and
...
/*
 * Releases the temporary b_cdata buffer in an l2arc header structure.
...
l2arc_release_cdata_buf(arc_buf_hdr_t *ab)
{
	...
	/*
	 * If the data was compressed, then we've allocated a temporary
	 * buffer for it, so now we need to release it.
	 */
	zio_data_buf_free(l2hdr->b_cdata, ab->b_size);
	l2hdr->b_cdata = NULL;
}
